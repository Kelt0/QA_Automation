...
public class UserTablePage extends AbstractPage{
    ...
    public List<WebElement> getDropdownOptionsForUser(String username) {
        return webDriver.findElements(By.xpath("//a[text()='" + username + "']/parent::td//preceding-sibling::td/select/option"));
    }

    public List<String> getDropdownTextsForUser(String username) {
        return getDropdownOptionsForUser(username).stream().map(WebElement::getText).collect(Collectors.toList());
    }

    public void clickCheckboxForUser(String username) {
        for (WebElement checkbox : checkboxes) {
            if (username.toLowerCase(Locale.ROOT).contains(checkbox.getAttribute("id").toLowerCase(Locale.ROOT))) {
                checkbox.click();
            }
        }
    }
